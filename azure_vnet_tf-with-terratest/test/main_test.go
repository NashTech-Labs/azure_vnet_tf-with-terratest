...
func TestTerraform_azure_virtualNetwork(t *testing.T) {
	...
	t.Run("vnet_resource_address_space_is_matching", func(t *testing.T) {
		assert.Equal(t, expectedVnetAddressSpace, actualResponse.Properties.AddressSpace.AddressPrefixes[0])
	})
...
func Deserialize(resp armresources.ClientGetByIDResponse) ReponseBase {
